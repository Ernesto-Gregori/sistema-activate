...
## 2. Respaldo de partidas

La partida se guarda automáticamente. Para respaldo manual:

- Abrir DevTools (`F12`) → **Aplicación** → **Almacenamiento local**.
- Copiar el valor de `dilo_como_es_save`.

También se puede usar el botón **💾 Guardar** y luego exportar el almacenamiento.

---

## 3. Restauración de una partida guardada
...
- Para borrar todos los datos (partida + preguntas + equipos):
  - Abrir DevTools → **Aplicación** → **Almacenamiento local** → **Limpiar todo**.

---

## 6. Soporte y actualizaciones

- El código está completamente contenido en un solo archivo HTML.
- Para actualizar, reemplazar el archivo.
